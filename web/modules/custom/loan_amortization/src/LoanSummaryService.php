...
/**
 * LoanSummaryService is a Drupal 8 service which implements loanSummary, loanPayments, PMT functions.
 */
...
class LoanSummaryService {
  ...
  /**
   * Constructs a HireOrderProcessor object.
   *
   ...
   *   The language manager.
   */
  public function __construct(ConfigFactoryInterface $config, LanguageManagerInterface $language_manager) {
  ...
  public function loanSummary ($loan_amount, $annual_rate,
                               $loan_period, $annual_payments,
                               $start_of_loan, $optional_payment) {
    ...
    $config_labels = $this->configFactory->get(static::SETTINGS_LABELS);
  ...
  public function loanPayments($loan_amount, $annual_rate,
                               $loan_period, $annual_payments,
                               $start_of_loan, $optional_payment) {
    $rate = $annual_rate / $annual_payments;
    $nper = $loan_period * $annual_payments;
