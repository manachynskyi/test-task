...
use Drupal\loan_amortization\LoanSummaryService;
...
class LoanAmortizationForm extends FormBase {
   ...
   * The renderer service.
   *
   * @var \Drupal\loan_amortization\LoanSummaryService
   */
   ...
   *   The renderer service.
   * @param \Drupal\loan_amortization\LoanSummaryService $loan_summary
   *   Custom service for calculation.
   */
  public function __construct(LanguageManagerInterface $language_manager, Request $request, AccountInterface $current_user, RendererInterface $renderer, LoanSummaryService $loan_summary) {
    $this->languageManager = $language_manager;
    $this->request = $request;
    $this->currentUser = $current_user;
    $this->renderer = $renderer;
    $this->loanSummary = $loan_summary;
  }
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      ...
      $container->get('LoanSummaryService')
    );
  }
  ...
  */
  ...
  /**
  * Building a form of entering loan data, using labels and default
  * values from config files in depending on the current language.
  */
  ...
  */
  ...
  /**
   * In ajaxSubmitCallback function we're get from $form_state entered data, but
   * if there is GET parameter in URL, the variable is rewriting.
   * Calculating is executing by Drupal service. The result is inserted in corresponding
   * block by AjaxResponse.
   */
  ...
  public function ajaxSubmitCallback(array &$form, FormStateInterface $form_state) {
    ...
    // GET parameters
    $loan_amount_get = $this->request->query->get('loan_amount');
    $annual_rate_get = $this->request->query->get('annual_rate');
    $loan_period_get =$this->request->query->get('loan_period');
    $annual_payments_get = $this->request->query->get('annual_payments');
    $start_of_loan_get = $this->request->query->get('start_of_loan');
    $extra_payment_get = $this->request->query->get('extra_payment');
  ...
  public function validateMessage($var, $check, $message,AjaxResponse $ajax_response, $selector){
    if ( $var == 0){
      ...
      $messages = $this->renderer->render($status_message);
